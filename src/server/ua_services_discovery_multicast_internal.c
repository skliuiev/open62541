...
addMdnsRecordForNetworkLayer(UA_Server *server, const UA_String *appName,
    ...
    retval = UA_Discovery_addRecord(
        server, appName, &hostname, port, &path, UA_DISCOVERY_TCP, true,
        server->config.discovery.mdns.serverCapabilities,
        server->config.discovery.mdns.serverCapabilitiesSize, true);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(&server->config.logger, UA_LOGCATEGORY_NETWORK,
...
stopMulticastDiscoveryServer(UA_Server *server) {
    ...
    for(size_t i = 0; i < server->config.networkLayersSize; i++) {
        ...
        UA_StatusCode retval = UA_parseEndpointUrl(
            &server->config.networkLayers[i].discoveryUrl, &hostname, &port, &path);
...
filterServerRecord(size_t serverCapabilityFilterSize, UA_String *serverCapabilityFilter,
    ...
    // if the element has no capabilities defined, but the filter expects some, then do
    // not use this entry
    if(serverCapabilityFilterSize > 0 &&
       current->serverOnNetwork.serverCapabilitiesSize == 0)
        return false;
...
UA_Server_updateMdnsForDiscoveryUrl(UA_Server *server, const UA_String *serverName,
    ...
    UA_StatusCode addRetval = UA_Discovery_addRecord(
        server, serverName, &hostname, port, &path, UA_DISCOVERY_TCP, updateTxt,
...
UA_Discovery_addRecord(UA_Server *server, const UA_String *servername,
    ...
    // The servername is servername + hostname. It is the same which we get through mDNS
    // and therefore we need to match servername
    UA_StatusCode retval = UA_DiscoveryManager_addEntryToServersOnNetwork(
        server, fullServiceDomain, fullServiceDomain,
    ...
    if(retval != UA_STATUSCODE_BADALREADYEXISTS) {
        ...
        listEntry->serverOnNetwork.serverCapabilities =
            (UA_String *)UA_Array_new(listEntry->serverOnNetwork.serverCapabilitiesSize,
        ...
        UA_snprintf(newUrl, 10 + hostnameLen + 8 + path->length + 1,
                    "opc.tcp://%.*s:%d%s%.*s", (int)hostnameLen, hostname->data, port,
                    path->length > 0 ? "/" : "", (int)path->length, path->data);
